...
static void renderprogressview(int w, int h, float bar, const char *text)   // also used during loading
{
    ...
    const float fh = 0.060f*std::min(w, h),
                fw = fh * 15,
...
static VARFP(vsync, 0, 0, 1, restorevsync());                      //vertical sync of framebuffer to refresh rate
static VARFP(vsynctear, 0, 0, 1, { if(vsync) restorevsync(); });   //toggles sdl2's adaptive sync function
...
static void getfpscmd(const int *raw)
{
    if(*raw)
    {
