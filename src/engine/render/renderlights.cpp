 ...
 * @brief render lights to deferred buffers
 *
...
//`g`-buffer `scale`
static VARFP(gscale, 25, 100, 100, gbuf.cleanupgbuffer()); //size of g buffer, approximately correlates to g buffer linear dimensions
static VARFP(gscalecubic, 0, 0, 1, gbuf.cleanupgbuffer()); //g-buffer scale cubic: use cubic interpolation for g buffer upscaling to screen output
static VARFP(gscalenearest, 0, 0, 1, gbuf.cleanupgbuffer()); //g buffer nearest neighbor interpolation
...
static VAR(csminoq, 0, 1, 1); //cascaded shadow maps in occlusion queries
static VAR(sminoq, 0, 1, 1);  //shadow maps in occlusion queries
    ...
    while(stack.size() > 0)
    {
        ...
        for(int g = 0; g < BatchFlag_NoShadow; ++g)
        {
            while(groups[g] >= lighttilebatch || (inside == outside && (groups[g] || !(flags & BatchFlag_NoSun))))
            {
                ...
                lightbatches.push_back(std::move(key));
                ++lightbatchrectsused;
            }
        }
...
static VAR(gdepthclear, 0, 1, 1); //toggles whether to clear the g depth buffer to 0000/1111 (black or white depending on gdepthformat) upon creation
static VAR(gcolorclear, 0, 1, 1); //toggles whether to clear the g buffer to 0,0,0,0 (black) upon creation
