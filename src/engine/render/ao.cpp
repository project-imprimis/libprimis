/**
 * @file ao.cpp
 * @brief screenspace ambient occlusion
 *
...
static GLuint aonoisetex = 0;

VARFP(ao, 0, 1, 1, { cleanupao(); cleardeferredlightshaders(); }); //toggles ao use in general
static FVARR(aoradius, 0, 5, 256);
static FVAR(aocutoff, 0, 2.0f, 1e3f);
static FVARR(aodark, 1e-3f, 11.0f, 1e3f);
static FVARR(aosharp, 1e-3f, 1, 1e3f);
static FVAR(aoprefilterdepth, 0, 1, 1e3f);
...
static VARP(aoblur, 0, 4, 7);
static VARP(aoiter, 0, 0, 4); //number of times to run ao shader (higher is smoother)
VARFP(aoreduce, 0, 1, 2, cleanupao());
VARF(aoreducedepth, 0, 1, 2, cleanupao());
static VARFP(aofloatdepth, 0, 1, 2, initwarning("AO setup", Init_Load, Change_Shaders));
static VARFP(aoprec, 0, 1, 1, cleanupao()); //toggles between r8 and rgba8 buffer format
static VAR(aodepthformat, 1, 0, 0);
static VARF(aonoise, 0, 5, 8, cleanupao()); //power or two scale factor for ao noise effect
VARFP(aobilateral, 0, 3, 10, cleanupao());
static FVARP(aobilateraldepth, 0, 4, 1e3f);
VARFP(aobilateralupscale, 0, 0, 1, cleanupao());
VARF(aopackdepth, 0, 1, 1, cleanupao());
static VARFP(aotaps, 1, 12, 12, cleanupao());
static VAR(debugao, 0, 0, 4);
