...
/**
 * @brief checks for existence of glext
 *
 ...
 *
 * @param ext the ext to check for
 */
...
static VARP(zoomoutvel, 0, 50, 500);
static VARP(zoomfov, 10, 42, 90);
static VARP(fov, 10, 100, 150);
static VAR(avatarzoomfov, 1, 1, 1);
static VAR(avatarfov, 10, 40, 100);
static FVAR(avatardepth, 0, 0.7f, 1);
 ...
 * 400: Cube/RE
 */
static VARP(invmouse, 0, 0, 1); //toggles inverting the mouse
...
static void blendfogoverlay(int fogmat, float below, float blend, vec &overlay)
{
    switch(fogmat&MatFlag_Volume)
    {
        case Mat_Water:
        {
            const bvec &wcol = getwatercolor(fogmat),
                       &wdeepcol = getwaterdeepcolor(fogmat);
            const int wfog = getwaterfog(fogmat),
                      wdeep = getwaterdeep(fogmat);
            const float deepfade = std::clamp(below/std::max(wdeep, wfog), 0.0f, 1.0f);
...
static void clipminimap(ivec &bbmin, ivec &bbmax, const std::array<cube, 8> &c, const ivec &co = ivec(0, 0, 0), int size = rootworld.mapsize()>>1)
{
    for(int i = 0; i < 8; ++i)
    {
        ivec o(i, co, size);
