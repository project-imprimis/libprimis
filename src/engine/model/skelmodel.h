...
struct skelmodel : animmodel
{
    ...
    class skelmesh : public Mesh
    {
        public:
            ...
            int genvbo(std::vector<GLuint> &idxs, int offset, std::vector<vvertg> &vboverts, int *htdata, int htlen);
    ...
    class skeleton
    {
        ...
        private:
            ...
            struct tag
            {
                ...
                tag(std::string_view initname, int initbone, matrix4x3 initmatrix) : name(initname), bone(initbone), matrix(initmatrix) {}
            };
            std::vector<tag> tags;
     ...
     */
    skelpart &addpart();
    meshgroup *loadmeshes(const std::string &key, float smooth = 2);
