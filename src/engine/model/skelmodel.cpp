...
skelmodel::skelanimspec &skelmodel::skeleton::addskelanim(std::string_view name, int numanimframes, int animframes)
{
    skelanims.push_back({name.data(), numanimframes, animframes});
...
bool skelmodel::skeleton::addtag(std::string_view name, int bone, const matrix4x3 &matrix)
{
    ...
    {
        tags.emplace_back(name, bone, matrix);
    }
    return true;
}
...
dualquat skelmodel::skeleton::interpbone(int bone, const std::array<framedata, maxanimparts> &partframes, const AnimState *as, const uchar *partmask) const
{
...
void skelmodel::skelmeshgroup::bindbones(const vvertgw *vboverts)
{
    meshgroup::bindbones(vboverts->weights.data(), vboverts->bones.data(), vertsize);
}
...
void skelmodel::skelmeshgroup::bindvbo(const AnimState *as, const part *p, const vbocacheentry &vc)
{
    if(!skel->numframes)
    {
