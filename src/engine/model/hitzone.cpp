...
void skelhitdata::build(skelmodel::skelmeshgroup *g, const uchar *ids)
{
    ...
    std::map<skelzoneinfo, skelzoneinfo> infomap;
    ...
    for(int i = 0; i < std::min(static_cast<int>(g->meshes.size()), 0x100); ++i)
    {
        ...
        for(int j = 0; j < m->numtris; ++j)
        {
            ...
            skelzonekey key(m, t);
            infomap.insert_or_assign(key, key);
            skelzoneinfo &zi = infomap[key];
    ...
    for(uint i = leafzones; i < info.size(); i++)
    {
        ...
        for(int j = 0; j < static_cast<int>(sizeof(deps.bones)); ++j)
        {
            ...
            infomap.insert_or_assign(dep, dep);
            skelzoneinfo &zj = infomap[dep];
            zj.parents++;
            zi.children.push_back(&zj);
        }
    }
