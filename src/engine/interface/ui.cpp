/**
 * @file ui.cpp
 * @brief cubescript user interfaces and menu functionality
 *
...
#undef LOOP_CHILD_RANGE
    ...
    void inituicmds()
    {
        ...
        #define DOSTATE(flags, func) \
            addcommand("ui" #func, reinterpret_cast<identfun>(+[] (const uint *t, const uint *f) \
            { \
                executeret(buildparent && buildparent->haschildstate(flags) ? t : f);\
            }), "ee", Id_Command); \
            addcommand("ui" #func "?", reinterpret_cast<identfun>(+[] (tagval *t, tagval *f) \
            { \
        ...
        addcommand("uifocus", reinterpret_cast<identfun>(+[] (const uint *t, const uint *f)
        {
            executeret(buildparent && TextEditor::focus == buildparent ? t : f);
        }), "ee", Id_Command);
        addcommand("uifocus?", reinterpret_cast<identfun>(+[] (tagval *t, tagval *f)
        {
        ...
        addcommand("uiclamp", reinterpret_cast<identfun>(+[] (const int *left, const int *right, const int *top, const int *bottom)
        {
            if(buildparent)
            {
        ...
        addcommand("uispace", reinterpret_cast<identfun>(+[] (const float *spacew, const float *spaceh, const uint *children)
        {
        ...
        addcommand("uiclip", reinterpret_cast<identfun>(+[] (const float *clipw, const float *cliph, const uint *children)
        {
