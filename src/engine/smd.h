    ...
    struct smdmeshgroup : skelmeshgroup
    {
        ...
        void readnodes(stream *f, char *buf, size_t bufsize, vector<smdbone> &bones)
        {
            while(f->getline(buf, bufsize))
            {
                ...
                if(id < 0 || id > 255 || parent > 255 || !name[0])
                {
                    continue;
                }
                while(!bones.inrange(id))
                {
                    bones.add();
                }
            ...
            while(f->getline(buf, bufsize))
            {
                ...
                for(int i = 0; i < 3; ++i)
                {
                    ...
                    int parent = -1,
                        numlinks = 0,
                        len = 0;
                    if(sscanf(curbuf, " %d %f %f %f %f %f %f %f %f %d%n", &parent, &key.pos.x, &key.pos.y, &key.pos.z, &key.norm.x, &key.norm.y, &key.norm.z, &key.tc.x, &key.tc.y, &numlinks, &len) < 9)
                    {
                        goto endsection;
                    }
                    curbuf += len;
                    ...
                    for(; numlinks > 0; numlinks--)
                    {
                        int bone = -1, len = 0;
                        float weight = 0;
                        if(sscanf(curbuf, " %d %f%n", &bone, &weight, &len) < 2)
                        {
                            break;
                        }
                        curbuf += len;
                        ...
                        {
                            sorted = c.addweight(sorted, weight, bone);
                        }
                    }
                    if(tweight < 1)
                    {
                    ...
                    if(pweight > 0)
                    {
                        sorted = c.addweight(sorted, pweight, parent);
                    }
                    c.finalize(sorted);
                    key.blend = curmesh->mesh->addblendcombo(c);
                    int index = verts.access(key, curmesh->verts.length());
                    if(index == curmesh->verts.length())
                    {
                        curmesh->verts.add(key);
                    }
        ...
        void readskeleton(stream *f, char *buf, size_t bufsize)
        {
            ...
            while(f->getline(buf, bufsize))
            {
                ...
                if(sscanf(curbuf, " time %d", &frame) == 1)
                {
                    continue;
                }
                else if(!strncmp(curbuf, "end", 3))
                {
                    break;
                }
                else if(frame != 0)
                ...
                {
                    b.base.mul(skel->bones[b.parent].base, dq);
                }
        ...
        int readframes(stream *f, char *buf, size_t bufsize, vector<smdbone> &bones, vector<dualquat> &animbones)
        {
            ...
            while(f->getline(buf, bufsize))
            {
                ...
                {
                    for(; lastbone < skel->numbones; lastbone++)
                    {
                        animbones[frame*skel->numbones + lastbone] = animbones[lastbone];
                    }
